            ...
            <div className="flex-1 flex flex-col h-screen overflow-hidden">
                <header className="bg-white border-b border-slate-200 p-4 flex items-center justify-between shadow-sm z-30">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <button onClick={saveToHistory} className="px-4 py-2 bg-indigo-600 text-white rounded-xl text-[9px] font-black uppercase tracking-widest shadow-lg shadow-indigo-600/20 active:scale-95 transition-all"><Save className="w-3.5 h-3.5 inline mr-1" /> {t.common?.save || 'Guardar'}</button>
                    </div>
                </header>
                            ...
                            <div className="space-y-3 max-h-[300px] overflow-y-auto custom-scrollbar">
                                {printerDevices.length === 0 ? (
                                    <div className="text-center py-8 text-slate-400">
                                        ...
                                        <p className="text-[9px] mt-2">{t.settingsPage?.printerBluetoothHint || 'Asegúrate de haber vinculado tu impresora en los ajustes del sistema.'}</p>
                                    </div>
                                ) : (
