        ...
        <div className="fixed inset-0 z-[9999] flex items-start pt-10 md:pt-20 justify-center bg-slate-900/98 p-4 animate-fadeIn overflow-y-auto">
            <div className={`w-full max-w-md bg-white rounded-3xl shadow-2xl overflow-hidden border-4 ${isCritical ? 'border-red-600 animate-pulse-border' : 'border-yellow-400'}`}>
                ...
                <div className="p-4 bg-white border-t border-slate-100 flex flex-col gap-3">
                    ...
                    >
                        ENTENDIDO, CERRAR POR AHORA
                    </button>
                    <button
                        onClick={() => {
                            const snoozeUntil = Date.now() + 3 * 60 * 60 * 1000;
                            localStorage.setItem('license_reminder_snooze', snoozeUntil.toString());
                            setShowModal(false);
                        }}
                        className="w-full py-3 rounded-xl border-2 border-slate-200 text-slate-500 font-black uppercase text-[10px] md:text-xs tracking-widest hover:bg-slate-50 transition-all flex items-center justify-center gap-2 active:scale-95"
                    >
                        <i className="fa-regular fa-clock"></i> POSPONER (AVISARME EN 3 HORAS)
                    </button>
                </div>
