...
const supabaseKey = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InNhbWdwbmN6bHpueW5uZmhqamZmIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc3MjE2NTU2NCwiZXhwIjoyMDg3NzQxNTY0fQ.5nN1gCgX7_4K0Rk2QZ-tAOFhP9pS1hVMBXX8mZqSGy4'; // Service Role for Admin Updates
...
async function repairBalances() {
    ...
    for (const client of clients) {
        ...
        // Fix client balance
        const theoreticalBalance = Math.max(0, totalClientDebt - totalClientPaid);
        if (Math.abs(Number(client.current_balance || 0) - theoreticalBalance) > 1.0) {
            await supabase.from('clients').update({
                current_balance: theoreticalBalance
            }).eq('id', client.id);
            updatedClientsCount++;
            console.log(`Corregido Cliente: ${client.name} | De: $${client.current_balance} -> $${theoreticalBalance}`);
        }
    }
