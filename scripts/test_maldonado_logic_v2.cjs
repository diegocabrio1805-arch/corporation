...
async function run() {
    ...
    // Simulated parsing done by App.tsx handleRealtimeData map
    const mappedLogs = logs.map(l => ({
        ...l,
        loanId: l.loan_id,
        clientId: l.client_id,
        isOpening: l.is_opening !== undefined ? l.is_opening : false,
        type: l.type
    }));

    const total = calculateTotalPaidFromLogs(loans[0], mappedLogs);
    console.log("TOTAL HELPER CJS:", total);
}
run();
